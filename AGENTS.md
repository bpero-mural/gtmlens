...
The authenticated app must be Livewire-first for page bodies and interactive UX.

- Authenticated page bodies must be implemented as Livewire components. Blade files under `resources/views/pages` should stay as thin layout wrappers that mount a Livewire panel.
...
- Classic Blade GET/POST is acceptable for authentication/session actions, file downloads, and external links.
...
Run `docker compose exec app php artisan salesforce:metadata-probe stage`, `docker compose exec app php artisan salesforce:source-retrieve stage`, or `docker compose exec app php artisan salesforce:source-normalize --latest --orgAlias=stage` only when an authenticated Salesforce CLI alias named `stage` is intentionally available.
